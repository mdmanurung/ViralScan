...
### HHV-6B provides an annotation-matched comparison that isolates the counting model

The EBV comparison conflates counting model with reference coverage, because ViralScan and STARsolo used different numbers of EBV loci. HHV-6B provides the one benchmark case where this confound is absent: in the Serratus combined reference, HHV-6B is encoded as 97 single-gene contigs (one gene per contig, no overlapping loci), so both tools count against identical annotation. We compared STARsolo 2.7.11b (GeneFull) and ViralScan on the HHV-6B CAR-T sample (SRR20710641) over the shared anchor of 3,517 cells (intersection of STARsolo filtered cells and ViralScan per-cell barcodes). On a harmonised unique-count layer (STARsolo unique-integer counts versus ViralScan pre-multimapping unique counts), ViralScan recovered 988 HHV-6B UMI versus 519 for STARsolo (1.9x). The ratio was unchanged under a two-step host-filter design (984 versus 519), and STARsolo was identical under both reference strategies (519 UMI), confirming that the reference-strategy axis is negligible. Because the annotation is identical here, this 1.9x — unlike the EBV difference — reflects the pseudoalignment counting model recovering reads that STARsolo's unique-integer assignment discards, rather than differences in reference coverage. Adding ViralScan's multimapping correction raised the count to 1,496 UMI (+51% over the unique layer; 2.9x STARsolo).

Two caveats bound this result. First, it rests on a single virus: the EBV and HSV-1 rows of the same 2x2 benchmark could not yield a clean aligner comparison because most EBV and HSV-1 genes are annotated as CDS-only records in the Serratus combined GTF, without the exon features STARsolo's GeneFull mode requires (STAR Methods). Second, HHV-6A and HHV-6B share approximately 95% nucleotide identity; the unique-count layer excludes shared-region multimappers by construction, and ViralScan additionally flags such sibling cross-mapping in its output when one member of a closely related pair dominates, but residual cross-mapping cannot be fully excluded. We therefore report HHV-6B as a single annotation-matched data point, consistent with our broader position that the STARsolo comparison is not a general claim of aligner superiority.
...
The STARsolo comparison should not be interpreted as a general claim that one quantifier is superior. On the 1,906 matched EBV cells, ViralScan and STARsolo ranked per-cell EBV burden similarly but assigned reads to different EBV genes. The divergence is consistent with differences in EBV annotation and counting model, especially for LMP-1 and EBNA-family features. This result argues for explicit reporting of viral reference annotations in single-cell viral RNA analyses. The one exception where annotation was matched — HHV-6B, where both tools counted against identical single-gene contigs — showed ViralScan recovering 1.9-fold more unique viral UMI, isolating a counting-model effect from the annotation-coverage effects that dominate the EBV and HSV-1 comparisons.
...
ViralScan source code is available at https://github.com/mdmanurung/ViralScan under the MIT licence. A versioned software DOI (distinct from the reference-data DOI below) will be assigned upon archiving the v2.5.0 GitHub release on Zenodo (pending). Reference-data archive (viral GTF annotations and index): DOI 10.5281/zenodo.20112332. Public datasets used in the benchmarks are available from GEO/SRA: GSE210063 (SRR20710641; HHV-6B CAR-T), GSE158275 (SRR12682296; EBV LCL), and GSE123782 (SRR8315713; HSV-1 fibroblasts). The two COVID-era clinical scRNA-seq libraries used in the SARS-CoV-2 specificity analysis (internal identifiers LUM-SJ-x213-g and LUM-SJ-x216-g) are not yet publicly deposited. **[AUTHORS: add the deposition accession or controlled-access statement for these libraries, and cite the IRB/ethics approval (institution and protocol number) and informed-consent basis under which they were collected — no ethics statement currently appears in the manuscript and this is required by all target venues.]** Benchmark scripts, figure-generation code, and manuscript figures are included in the repository under `scripts/` and `docs/figures/`.
...
#### Reference-strategy and annotation-matched aligner comparison

For the HHV-6B annotation-matched comparison, STARsolo 2.7.11b (GeneFull mode) and ViralScan were each run on SRR20710641 under two reference strategies — a single combined host+viral index and a two-step host-filter-then-viral design — and compared on the intersection of STARsolo filtered cells and ViralScan per-cell barcodes (3,517 cells). Counts were harmonised to a common layer (STARsolo unique-integer counts versus ViralScan pre-multimapping unique counts) restricted to shared target features. EBV and HSV-1 were excluded from the aligner comparison because most of their genes are annotated as CDS-only in the Serratus combined GTF and are therefore not countable by STARsolo GeneFull, which builds transcript models from exon records only. Harmonisation code is provided at `analysis/reference_strategy_benchmark/scripts/harmonize_2x2.py`.
...
12. Bost, P., Giladi, A., Liu, Y., et al. (2020). Host-viral infection maps reveal signatures of severe COVID-19 patients. *Cell*, 181, 1475-1488.e12. https://doi.org/10.1016/j.cell.2020.05.006 <!-- [AUTHORS: verify ViralTrack citation details before submission] -->
