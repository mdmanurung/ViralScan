...
**2.3.0**. Available subcommands: `build-ref`, `data fetch`, `evidence`,
`rerun-multimap`, `hostresponse`.
...
**Tip:** run with the default (`equal`) first for fast results, then rerun with
`--multimap-method host-conservative` or `em` for refinement without repeating
pseudoalignment.
...
| `--detection-threshold N` | | from config or `1` | Min UMI count to call a cell virus-positive |
...
```bash
# Minimal — run with defaults from config
