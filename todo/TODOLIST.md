...
| medium | **EVE Phase B coordinate bug** — fixed via `_is_chromosome_subject()`; clone/scaffold subjects flagged `subject_not_chromosome`. — **DONE 2026-07-21** (607a340). | bug | done | 2026-07-21 |
| medium | **EVE chromosome-name normalization** — added `_normalize_chrom()` (GTF keys + queries) and `_warn_namespace()`. — **DONE 2026-07-21** (607a340). | bug | done | 2026-07-21 |
| low | **EVE Phase B parse path untested** — added parse-path, normalization, and namespace-warning tests (8 pass). — **DONE 2026-07-21** (607a340). | test | done | 2026-07-21 |
| low | **EVE SLURM error masking** — keep stderr in a log + visible WARN instead of `2>/dev/null \|\| true`. — **DONE 2026-07-21** (607a340). | bug | done | 2026-07-21 |
| medium | **Publication hygiene — residual institutional paths** — DOC refs scrubbed (`evonk` python path in `PLAN.md`/`RUNBOOK.md`; RUNBOOK conda-env row → `$VS_CONDA_ENV`). **STILL OPEN:** ~14 functional SLURM/infra scripts (`covid_viralscan/scripts/*.sh`, `scripts/*.sh`) and `reference_manifest.json` embed `VS_CONDA_ENV=/exports/.../evonk/...` as a *default* — scrubbing needs per-script env-var-ization (breaking-risk) or excluding them from the shipped artifact. Decide ship-scope first. | hygiene | partial | 2026-07-21 |
| low | **Gitignore data-lineage telemetry** — added rules for `.living/log/data-lineage/` and `.data-lineage-status-*.json`. — **DONE 2026-07-21**. | infrastructure | done | 2026-07-21 |
| low | **PLAN.md + docs consistency** — added a PLAN.md DONE row for the vignette suite. `basic_usage` refs left as accurate *history* (PLAN PR-7 entry) / non-shipping meta (`write-docs-prompt.md`). — **DONE 2026-07-21**. | docs | done | 2026-07-21 |
| low | **Primary-call code cleanup** — `resolve_count_matrix()` helper, dead branch dropped, double-sum + `_sum_axis1` reuse. — **DONE 2026-07-21** (f143d94). | refactor | done | 2026-07-21 |
| medium | **Open a PR** for `claude/pub-readiness-hygiene` → `main`; note the git SSH commit-signing key mismatch (`user.signingkey`) — recent commits may be unsigned. | process | open | 2026-07-21 |
