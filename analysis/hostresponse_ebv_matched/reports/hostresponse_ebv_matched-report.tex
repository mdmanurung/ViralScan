...
inflated it from about 0.72 to \SciVal{\AUCMean}{0.866}, but did not invent it.
...
The five that survive depth adjustment (E-value $\geq 2$; the depth-robust core that
carries the AUC-0.72 signal) form a biologically coherent, not random, set --- they
map onto host programs EBV is known to modulate. Up in EBV-positive cells:
\textit{LTA} (lymphotoxin-$\alpha$, a TNF-family NF-$\kappa$B target --- the most
direct link, since EBV's LMP1 constitutively activates NF-$\kappa$B), \textit{ING1} (a
p53-pathway tumor suppressor), and \textit{MT-ND4L} (a mitochondrial
oxidative-phosphorylation subunit, consistent with EBV metabolic reprogramming). Down
in EBV-positive cells: \textit{EVI2B} (myeloid differentiation) and \textit{MACROD2}
(DNA-damage ADP-ribosylhydrolase). Caveat: \textit{MT-ND4L} is mitochondrial, and
mitochondrial-read fraction is also a QC covariate, so it is the one hit most in need
of a mito-fraction control (see \texttt{depth\_robust\_genes.md}).
