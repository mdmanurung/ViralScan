...
\textbf{Reproducibility note.} These metrics reflect a leakage-corrected re-run
in which HVG selection was moved inside the cross-validation split; an earlier
version that selected HVGs on the full dataset gave slightly lower discrimination
(AUC 0.845). Selecting features per fold, on the balanced training cells, removed
the leak and modestly \emph{raised} performance, because features chosen on the
balanced training set track the EBV contrast better than those chosen on the
majority-dominated full dataset. The stable-gene set was unchanged.
