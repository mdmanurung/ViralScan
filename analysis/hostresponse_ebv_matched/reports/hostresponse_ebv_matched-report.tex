...
\SciVal{\AUCMean}{0.845} and a mean Matthews correlation coefficient (MCC) of
\SciVal{\MCCMean}{0.539} ($\pm\,\SciVal{\MCCSD}{0.059}$ across seeds; chance
...
\begin{itemize}
  ...
  1 is perfect agreement. It is more conservative than accuracy when the two
  classes differ in size, which is why we report it alongside AUC.
...
host genes (HVGs), the genes whose
expression varies most across cells. For each of six seeds
($\{0,1,10,42,100,1234\}$) the positive and negative cells were balanced and
depth-matched, the model trained on one split and evaluated on the held-out
cells, and sensitivity, specificity, balanced accuracy, AUC, and MCC recorded.
We report the mean and standard deviation across seeds. The intuition for
averaging over seeds is that a single train/test split can be lucky or unlucky;
the spread across seeds tells us how much of the headline number is signal versus
split-specific noise.

\textbf{Stable genes.} Randomized-lasso stability selection (seed 42) scored each
host gene by how often it survived; genes selected in $\geq$60\% of iterations are
the \SciVal{\NStableGenes}{15} stable host genes reported here.

\textbf{Reproducibility note.} We verified reproducibility by re-running the
procedure with the same fixed seeds. Sensitivity, specificity, and balanced
accuracy reproduced exactly and the stable-gene set was identical; AUC reproduced
to three decimals, so we quote it at that precision.
...
EBV-negative cells with a mean AUC of \SciVal{\AUCMean}{0.845}, far above the
...
imbalance, was \SciVal{\MCCMean}{0.539} ($\pm\,\SciVal{\MCCSD}{0.059}$),
...
EBV-negative ones: mean sensitivity was \SciVal{\SensitivityMean}{0.824} versus
mean specificity of \SciVal{\SpecificityMean}{0.710}, for a balanced accuracy of
\SciVal{\BalancedACCMean}{0.767}. The gap matters for interpretation. Some cells
...
mean AUC of \SciVal{\AUCMean}{0.845} and a mean MCC of \SciVal{\MCCMean}{0.539},
...
the lower specificity of \SciVal{\SpecificityMean}{0.710} is consistent with some
...
\SciVal{\SensitivityMean}{0.824}, specificity \SciVal{\SpecificityMean}{0.710},
balanced accuracy \SciVal{\BalancedACCMean}{0.767}, AUC \SciVal{\AUCMean}{0.845},
and MCC \SciVal{\MCCMean}{0.539} ($\pm\,\SciVal{\MCCSD}{0.059}$).
