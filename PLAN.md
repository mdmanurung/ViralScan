...
→ **Cell-calling + report-both-denominators** — **DONE** (2026-07-03). `cellcalling.py` (external CellRanger/STARsolo list | emptydrops via `emptydrops.R`+DropletUtils | knee | none); `detection.py` viral_summary reports over BOTH called cells (primary) and all barcodes. Fixes the recurring empty-droplet trap. 16 tests; suite 521 passed. **TODO**: expose `cell_calling`/`called_cells_file` via `menu.py`+`createconfig.py` (works via config getattr today).
