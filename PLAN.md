...
→ **covid_viralscan analysis** — Stage 2 **COMPLETE** (2026-07-02): index.idx 499 MB, 470,468 targets verified via `kallisto inspect`; t2g sanity all pass. Two bugs found+fixed en route: (1) chromosomal GTF seqnames ≠ cDNA FASTA headers → cDNA-level GTF (`gen_combined_cdna_gtf.py`); (2) `kallisto index` failed on duplicate accession `NC_002076.2` (Torque teno virus 1, byte-identical, in both anello+Serratus sets) → keep-first dedup + dedup guard (Step 2.6) added to `slurm_build_ref.sh`. Stage 3 quant array RUNNING (job 25138573, array 0–1). Stage 4 analysis pending. See `covid_viralscan/RUNBOOK.md`.
